...
// -- Cadastra (Endereço)
// usuario
// -- Cadastra Descarte  ( Eletroeletronica )
// -- Mintar NFT do PRODUTO
...
contract W3Recicle is AccessControl, ERC721, ERC721URIStorage, ERC721Enumerable {
    ...
    bytes32 public constant COLLECTPOINT = keccak256("COLLECTPOINT");
    ...
    mapping(uint256 => Product) public product; // Mapeia Produtos Criados

    function registerProduct(string memory model, string memory fabricant, string memory mei) public onlyLeader(){  //  Somente um Leader pode registrar o produto
        
    }

    function discard(string memory model, string memory fabricant, string memory mei) public onlyMember(){  // Somente um usuário pode descartar o eletroeletronico
        
    }

    function safeMintNFT(address to, string memory uri) public onlyRole(USER) {    // Somente um usuário pode mintar mintar uma NFT 
    ...
    constructor() ERC721("W3Recicle", "W3R") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(COLLECTPOINT, msg.sender);
    }

    modifier onlyLeader() {
        require(hasRole(COLLECTPOINT, msg.sender), "Caller is not a leader");
        _;
    }

    modifier onlyMember() {
        require(hasRole(USER, msg.sender), "Caller is not a member");
        _;
    }
